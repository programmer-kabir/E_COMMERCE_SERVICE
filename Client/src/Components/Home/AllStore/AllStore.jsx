    ...
    <section>
      <Content>
        ...
        <div className="md:flex justify-between">
          <h2 className="text-3xl mb-8 font-semibold border-s-2 md:border-s-3 border-[#F62977] pl-3">
            All Store
          </h2>
          <Link>
          ...
          >
            {stores.map((store, index) => (
              <SwiperSlide key={index}>
                <div>
                  <img
                    className="categoryImg w-full"
                    src={store.imgUrl}
                    alt=""
                  />
                  <p className="text-center pt-5 font-medium primaryColor">
                    {store.name}
                  </p>
                </div>
              </SwiperSlide>
